  ...
  class ShapingQueue {
    ...
    public:
      ...
      #define SHAPING_QUEUE_DEQUEUE(AXIS)                                                                  \
        ...
        static bool __O0 dequeue_x() { SHAPING_QUEUE_DEQUEUE(x) }
        ...
        static bool __O0 dequeue_y() { SHAPING_QUEUE_DEQUEUE(y) }
...
class Stepper {
  ...
  public:
    ...
    static constexpr uint32_t minimal_step_rate = (
      #ifdef CPU_32_BIT
        _MAX((STEPPER_TIMER_RATE) / HAL_TIMER_TYPE_MAX, 1) // 32-bit shouldn't go below 1
      #else

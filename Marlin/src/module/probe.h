...
public:
  ...
  static bool stow(const bool no_return=false)   { return set_deployed(false, no_return); }
     ...
     */
    TERN(PROUI_EX, static const, static constexpr) float _min_x(const xy_pos_t &probe_offset_xy = offset_xy) {
      return TERN(IS_KINEMATIC,
        (X_CENTER) - probe_radius(probe_offset_xy),
        _MAX((X_MIN_BED) + (PROBING_MARGIN_LEFT), (X_MIN_POS) + probe_offset_xy.x)
      );
    }
    TERN(PROUI_EX, static const, static constexpr) float _max_x(const xy_pos_t &probe_offset_xy = offset_xy) {
      return TERN(IS_KINEMATIC,
        (X_CENTER) + probe_radius(probe_offset_xy),
        _MIN((X_MAX_BED) - (PROBING_MARGIN_RIGHT), (X_MAX_POS) + probe_offset_xy.x)
      );
    }
    TERN(PROUI_EX, static const, static constexpr) float _min_y(const xy_pos_t &probe_offset_xy = offset_xy) {
      return TERN(IS_KINEMATIC,
        (Y_CENTER) - probe_radius(probe_offset_xy),
        _MAX((Y_MIN_BED) + (PROBING_MARGIN_FRONT), (Y_MIN_POS) + probe_offset_xy.y)
      );
    }
    TERN(PROUI_EX, static const, static constexpr) float _max_y(const xy_pos_t &probe_offset_xy = offset_xy) {
      return TERN(IS_KINEMATIC,
        (Y_CENTER) + probe_radius(probe_offset_xy),
    ...
    public:
      TERN(PROUI_EX, static const, static constexpr) bool can_reach(float x, float y) {
        #if IS_KINEMATIC
      ...
      TERN(PROUI_EX, static const, static constexpr) bool can_reach(const xy_pos_t &point) { return can_reach(point.x, point.y); }
    };
...
private:  //run_z_probes
  static bool probe_down_to_z(const_float_t z, const_feedRate_t fr_mm_s);
  static float run_z_probes(const bool sanity_check=true, const_float_t z_min_point=Z_PROBE_LOW_POINT, const_float_t z_clearance=Z_TWEEN_SAFE_CLEARANCE);
};

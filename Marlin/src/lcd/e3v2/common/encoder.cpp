...
EncoderState encoderReceiveAnalyze() {
  ...
  if (abs_diff >= ENCODER_PULSES_PER_STEP) {
    temp_diffState = temp_diff > 0
      ? TERN(PROUI_ITEM_ENC, direction1, TERN(REVERSE_ENCODER_DIRECTION, ENCODER_DIFF_CCW, ENCODER_DIFF_CW))
      : TERN(PROUI_ITEM_ENC, direction2, TERN(REVERSE_ENCODER_DIRECTION, ENCODER_DIFF_CW, ENCODER_DIFF_CCW));
